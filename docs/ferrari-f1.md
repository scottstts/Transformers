...
- **Profile** (`F1_PROFILE`):
  ...
  - robot: walk 4.8 m/s with 1.575 m steps, run 15.6 m/s with 3.8 m steps, preserving the original walking and running step rates. Running support occupies 25 % of each leg's cycle, near a sprinter's duty factor. Its gait rolls on a long sole (heel 0.43, toe 0.78 m from the ankle; the wing halves dock on the foot). It is tuned against human joint ranges: a walking vault, a sprinter's heel recovery and knee drive, and arms carried against the torso lean, along with parallel foot tracks, forward knee alignment, restrained torso motion and narrow arm pumps (robot-locomotion.md, "Carriage per robot");
