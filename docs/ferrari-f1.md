...
- **Profile** (`F1_PROFILE`):
  ...
  - robot: walk 4.8 m/s with 1.575 m steps, run 15.6 m/s with 3.8 m steps, preserving the original walking and running step rates. Running support occupies 25 % of each leg's cycle. Its gait rolls on a long sole (heel 0.43, toe 0.78 m from the ankle; the wing halves dock on the foot). The run uses continuous heel recovery into a descending foot sweep, narrow opposing arm pumps timed to thigh drive, and a torso whose flight is not pulled down by recovering feet. Walking retains its vault; both use parallel foot tracks and forward knee alignment (robot-locomotion.md, "Carriage per robot");
