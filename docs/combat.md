...
- **Movement takes the robot back** (`RobotCombat.release`, `ComboController.cancellable`): during a recovery, or once a move's chain window has been open 0.1 s with no click waiting, holding a direction ends the fight at once. The pose hands back to the gait over 0.16 s while the gait already moves the robot (the fight no longer owns it: `active` is false while `loose`). A click, the guard or the special from there re-plants the feet where the model shows them. Before, the fight owned the robot through the whole recovery and its fade (1.4 s after the truck's last move), so the keys seemed dead.
