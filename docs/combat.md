...
- **Truck finisher:** the charge takes 2.95 s, with the slam at 1.28 s and the loop/movement boundary at 2.1 s. The shorter drive feeds a side-offset overhead raise; the pelvis starts the downward effort before the spine and chest. Landing compression follows the blow, then the supporting-hand brace, release and two regathering steps. The axe stays low and dissolves before its main-hand IK releases into a matching empty-hand guard; recovery then lowers the arm. Wrist channels retain the holding orientation during release and unwind afterwards. Preserve the supporting hand's grab/release durations: compressing these exchanges produces joint-speed spikes even when the pose positions remain continuous.
