...
- The tiers are distance bands (`LOD_DISTANCE`: 26 / 70 m), capped at 18 soldiers in LOD0 and 48 in LOD1 (`LOD_CAP`): a crowd pressing round the robot would otherwise put ~85k triangles each on screen. `Horde.drawFor` culls soldiers to the frustum on the CPU, sorts them near to far and hands the list over; since the tiers are bands of a sorted list (and the caps only push the farther ones down a tier), each tier is one consecutive run of the buffers. One material per slot serves all tiers: a per-mesh `userData.base` (read with `onObjectUpdate`) offsets the instance index. Capacity is 128 drawn at once.
...
- **Alert**: a garrison is alerted when the target is inside the wall polygon, and called off when the target is more than 25 m beyond the barrier ring. Alerted soldiers light their blades and fight. At peace each walks its post's beat (`Post.beat` in plan.ts), rolling slowly (1.8 m/s) to each point, stopping there 0.6-2.8 s and looking about: patrols circle the yard (alternate ones the other way round), sentries pace along the wall inside each gate (2.2-6 s stops), pairs walk the lanes, guards pace before the hangars. A garrison standing still at its posts looked dead. Called off, they rejoin their beats at the nearest point. Beat points stand clear of every collider (tested).
...
- **Attacks**: at most `ATTACKERS` (4) swing at once. A slash lands at a fixed moment of its strike; it hits if the robot is within reach and inside a 60 degree cone of the soldier's heading. Soldiers only reach the robot while it is `present` (not in a special's cutscene, not airborne).
...
A garrison is 28 (twice the first forts', in forts four times the area). When a fight cuts it below 16, a wave rolls out of the hangar doors (the two alternate) one soldier every 0.6 s until it is whole again. At peace it refills one every 3 s. Refilling whenever the garrison was short made kills invisible.

## Fighting feel

The target is a musou brawler (One Piece: Pirate Warriors 3): crowds go down fast and fly, the robot answers the keys at once. A soldier has 70 health, so most take two blows. The robots' blows are wide and long (arcs 95-180°, reach 3.5-5.8 m) and throw hard enough that finishers launch. Controls: combat.md (buffered clicks, steering each move, movement cutting the recovery) and robot-locomotion.md (steering).
