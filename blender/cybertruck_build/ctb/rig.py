"""Robot skeleton: rest datums, forward kinematics, and Blender empties.

Rest pose = the standing robot, arms hanging straight, every bone's local axes
aligned with the character axes (+X left, -Y forward, +Z up). A pose gives each
bone a local rotation (about its joint) and a local slide (telescoping joints).
The robot's standing origin is at forward station ROBOT_F of the vehicle frame.
"""
import math
import bpy
from mathutils import Vector, Matrix, Quaternion, Euler
from .kit import V


# joint heights / lengths (metres)
ANKLE_Z = 0.40
SHIN = 1.38
THIGH = 1.32
HIP_Z = ANKLE_Z + SHIN + THIGH          # 3.10
HIP_X = 0.40
# fold datums the stand position derives from: the robot lies on its back with the hip joint at
# (FOLD_PELVIS_F, FOLD_PELVIS_Z) and the toe-cap bumper bottom at FOLD_SOLE_Z; it rises about its
# planted feet, so it stands where its ankles were in the truck
FOLD_PELVIS_F = -0.60
FOLD_PELVIS_Z = 0.90
FOLD_SOLE_Z = 0.36
ROBOT_F = FOLD_PELVIS_F + math.sqrt((THIGH + SHIN) ** 2 - (FOLD_PELVIS_Z - FOLD_SOLE_Z - ANKLE_Z) ** 2)
WAIST_Z = HIP_Z + 0.36                  # waist joint sits on the hip beam's socket
CHEST_Z = WAIST_Z + 0.38
SHOULDER_Z = CHEST_Z + 1.00
SHOULDER_X = 1.22
CLAV_X = 0.43                           # shoulder boom pivot, clear of the head well
UPPER = 1.08
FORE = 1.00
NECK_Z = CHEST_Z + 1.18
HEAD_Z = NECK_Z + 0.20

FINGERS = ('index', 'middle', 'ring', 'pinky')
PHAL = (0.13, 0.105, 0.085)


def bone_defs():
    """(name, parent, head (x, f, z) at rest)."""
    B = [
        ('pelvis', None, (0, 0, HIP_Z)),
        ('spine', 'pelvis', (0, 0, WAIST_Z)),
        ('chest', 'spine', (0, 0, CHEST_Z)),
        ('neck', 'chest', (0, 0.0, NECK_Z)),
        ('head', 'neck', (0, 0.0, HEAD_Z)),
    ]
    for S, s in (('L', 1), ('R', -1)):
        B += [
            ('clav.' + S, 'chest', (s * CLAV_X, 0, SHOULDER_Z)),              # yaw pivot of the shoulder boom
            ('boom.' + S, 'clav.' + S, (s * CLAV_X, 0, SHOULDER_Z)),          # telescope middle stage (slides along x)
            ('yoke.' + S, 'boom.' + S, (s * CLAV_X, 0, SHOULDER_Z)),          # telescope inner stage + shoulder socket
            ('upperarm.' + S, 'yoke.' + S, (s * SHOULDER_X, 0, SHOULDER_Z)),
            ('forearm.' + S, 'upperarm.' + S, (s * SHOULDER_X, 0, SHOULDER_Z - UPPER)),
            ('hand.' + S, 'forearm.' + S, (s * SHOULDER_X, 0, SHOULDER_Z - UPPER - FORE)),
            ('hip.' + S, 'pelvis', (s * HIP_X, 0, HIP_Z)),          # prismatic carriage (x)
            ('thigh.' + S, 'hip.' + S, (s * HIP_X, 0, HIP_Z)),
            ('shin.' + S, 'thigh.' + S, (s * HIP_X, 0, HIP_Z - THIGH)),
            ('foot.' + S, 'shin.' + S, (s * HIP_X, 0, ANKLE_Z)),
        ]
        wz = SHOULDER_Z - UPPER - FORE
        # fingers hang from the knuckle line; palm faces inward (-s x)
        for i, fn in enumerate(FINGERS):
            f0 = 0.096 - i * 0.064
            z = wz - 0.366
            B.append(('%s1.%s' % (fn, S), 'hand.' + S, (s * SHOULDER_X, f0, z)))
            z -= PHAL[0]
            B.append(('%s2.%s' % (fn, S), '%s1.%s' % (fn, S), (s * SHOULDER_X, f0, z)))
            z -= PHAL[1]
            B.append(('%s3.%s' % (fn, S), '%s2.%s' % (fn, S), (s * SHOULDER_X, f0, z)))
        B.append(('thumb1.' + S, 'hand.' + S, (s * (SHOULDER_X - 0.094), 0.09, wz - 0.10)))
        B.append(('thumb2.' + S, 'thumb1.' + S, (s * (SHOULDER_X - 0.105), 0.14, wz - 0.22)))
        B.append(('thumb3.' + S, 'thumb2.' + S, (s * (SHOULDER_X - 0.11), 0.165, wz - 0.32)))
    return B


class Skeleton:
    def __init__(self):
        self.defs = bone_defs()
        self.names = [d[0] for d in self.defs]
        self.parent = {d[0]: d[1] for d in self.defs}
        self.head = {d[0]: V(*d[2]) for d in self.defs}
        self.offset = {}
        for n, p, _ in self.defs:
            self.offset[n] = self.head[n] - (self.head[p] if p else Vector((0, 0, 0)))
        self.children = {n: [] for n in self.names}
        for n, p, _ in self.defs:
            if p:
                self.children[p].append(n)

    def rest_world(self, name):
        return Matrix.Translation(self.head[name])

    def fk(self, pose, root=None):
        """pose: name -> (Quaternion rot, Vector slide) local. root: world matrix
        for the pelvis joint frame (defaults to rest at ROBOT_F).
        Returns name -> world Matrix of each joint frame."""
        W = {}
        for n in self.names:
            rot, slide = pose.get(n, (Quaternion(), Vector()))
            p = self.parent[n]
            if p is None:
                base = root if root is not None else Matrix.Translation(self.head[n] + V(0, ROBOT_F, 0))
                W[n] = base @ rot.to_matrix().to_4x4()
            else:
                W[n] = W[p] @ Matrix.Translation(self.offset[n] + slide) @ rot.to_matrix().to_4x4()
        return W


def rest_pose():
    return {}


# ------------------------------------------------------------- blender sync

def sync_empties(skel, coll, W=None):
    """Create/refresh one empty per bone at world matrices W (rest if None)."""
    objs = {}
    for n in skel.names:
        name = 'bone.' + n
        o = bpy.data.objects.get(name)
        if o is None:
            o = bpy.data.objects.new(name, None)
            coll.objects.link(o)
            o.empty_display_type = 'PLAIN_AXES'
            o.empty_display_size = 0.12
        o.rotation_mode = 'QUATERNION'
        objs[n] = o
    for n in skel.names:
        o = objs[n]
        p = skel.parent[n]
        o.parent = objs[p] if p else None
        o.matrix_parent_inverse = Matrix.Identity(4)
    Wd = W or skel.fk({})
    for n in skel.names:
        o = objs[n]
        p = skel.parent[n]
        local = (Wd[p].inverted() @ Wd[n]) if p else Wd[n]
        o.matrix_basis = local
    return objs
