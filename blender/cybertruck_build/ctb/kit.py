"""Hard-surface modeling kit for the Cybertruck transformer build.
...
design numbers read naturally: nose at f = +2.84, tail at f = -2.84.
...
def boolean(o, cutter, op='DIFFERENCE', apply=True, remove_cutter=True):
...
def cutter_obj(mesh, name='cutter'):
    o = obj_from_pydata(name, mesh[0], mesh[1], ['plastic'], smooth=False)
    fix_normals(o)
    return o
...
def append_builder(o, b):
    ...
    me = o.data
    names = [m.name[3:] if m and m.name.startswith('ct.') else None for m in me.materials]
    remap = []
    for s in b.slots:
        if s not in names:
            me.materials.append(mats.get(s))
...
def cut_mesh(mesh, cutters, op='DIFFERENCE'):
    ...
    o = obj_from_pydata('tmp.cut', mesh[0], mesh[1], ['plastic'], smooth=False)
    fix_normals(o)
    for c in cutters:
