...
# ------------------------------------------------------------------ car-mode containment
def inside_mask(bvh, pts, up=Vector((0.0, 0.0, 1.0)), eps=1e-5, max_hits=32):
    """Ray-parity inside test against a closed mesh (BVH), one ray per point."""
    out = []
    for p in pts:
        n, o = 0, Vector(p)
        for _ in range(max_hits):
            hit = bvh.ray_cast(o, up)
            if hit[0] is None:
                break
            n += 1
            o = hit[0] + up * eps
...
def containment_report(shell, objs, limit=0.004, sample=600, floor_z=0.13):
    """Robot objects poking out of the car shell: [(name, fraction out, max distance out)].
    shell: a closed mesh object in world space (the car's outer body)."""
    dg = bpy.context.evaluated_depsgraph_get()
    sm = WMesh(shell, dg)
    bvh = sm.bvh
    rows = []
    for o in objs:
        ...
        if not len(m.v):
            continue
        pts = sample_pts(m.v, sample)
        ins = inside_mask(bvh, pts)
        worst, nout = 0.0, 0
        for p, i in zip(pts, ins):
            if i:
                continue
            q, nrm, _, d = bvh.find_nearest(Vector(p))
            if nrm.z < -0.8 and p[2] < floor_z:
                continue            # under the body's bottom skin: the floor hides it
            if d > limit:
                nout += 1
                worst = max(worst, d)
        if nout:
            rows.append((o.name, round(nout / len(pts), 3), round(worst, 3)))
...
def sample_pts(v, k):
    ...
    idx = np.linspace(0, len(v) - 1, k).astype(int)
    return [tuple(v[i]) for i in idx]
